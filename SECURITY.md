...
proof of concept helps. If you are unsure whether something counts, report it anyway.
...
advisory unless you would rather stay anonymous. There is no bug bounty.

Please give us 90 days before public disclosure, or less if we ask for less. If we go quiet for
30 days, disclose.
...
against your own self-hosted instance is always fine.

## Areas worth your attention

Parts of the system where a bug would matter most:
...
- **Role enforcement on write tools.** Write tools pass `requires: 'editor'`; reads are open to
  ...
  own membership, and those guards exist to prevent lockout and privilege escalation.
...
- **User-defined (brain-authored) tools.** A page under `tools/` in a brain becomes an MCP tool
  ...
  custom tool cannot exceed the caller's existing access or shadow a first-party tool. Breaking
  any part of that, especially by escaping interpolation into an executed path or reaching a
  write, is what we most want to hear about.
...
The things most likely to hurt you:
