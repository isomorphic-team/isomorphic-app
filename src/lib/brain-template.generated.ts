...
**The same change across many pages** is one \`set_fields\` call with the list of paths, not one \`write_page\` each: it lands as a single commit and a single changelog line, and skips pages that already carry the values. This is the tool for "mark these done", "put this owner on everything in the folder", or clearing a field you have stopped using.
...
- **Setting fields.** \`write_page\`'s \`fields\` changes a page's metadata without touching its text, and \`set_fields\` does the same across a list of pages in one commit. Reach for these instead of rewriting a page to change a \`done:\` or an \`owner:\`.
...
export const WIKI_LOG_MD = `# Log

Append-only changelog of brain edits. Newest entries on top.

<!-- The librarian tools append an entry here on every page create, update, move, and delete. -->
`;

export const WIKI_OPEN_QUESTIONS_MD = `# Open questions

Things this brain knows it doesn't know. Agents add bullets when raw material raises a question that no wiki page answers; when a later page resolves one, strike it through and link the page that answers it.
