...
| #   | Item                                           | Blocking?                       | Risk                                        |
| --- | ---------------------------------------------- | ------------------------------- | ------------------------------------------- |
| W1  | Search relevance and a per-page hit cap        | Yes, for probe to mean anything | High: changes the read path every tool uses |
...
### W1. Search relevance, and a per-page hit cap

**Why.** Two separate defects in `searchIndex`, both evidenced above.

_Relevance._ A substring `LIKE` cannot answer a question, and `ORDER BY path` is not
an ordering. Fixing this improves every read an agent makes, not just the loop, and it
is the only way probe gains a real error signal.

_The hit cap._ The result loop has **no per-page limit**:
...
One page can consume the entire 50-hit budget. Observed in the probe run: single pages
returned 17 and 15 matching lines for one query. Combined with `ORDER BY path`, a
common term on a large brain returns 50 lines from whichever page sorts earliest, and
every other page is invisible. This is a scale bug: the measured brain never truncated
(0 inconclusive verdicts across 34 probes), so it is latent, not live.

**Shape.**

1. **Verify D1 supports FTS5 virtual tables before designing around them.** Check
   Cloudflare's current D1 documentation rather than model memory, and record the
   answer in [`docs/references.md`](../references.md) either way.
2. If FTS5 is available: an FTS table over `brain_pages.content`, BM25 ordering, built
   through the existing derived-rebuild machinery. This is a new derived structure, so
   it needs an `INDEX_SCHEMA_VERSION` bump and must adopt the budget/cursor/
   advance-the-marker-only-when-done shape that `rebuildDerivedFromStore` already uses.
   A whole-brain pass that runs to completion inline is how a large brain becomes
   permanently unreadable.
3. If FTS5 is not available: tokenize the query, `AND` the terms in SQL, and rank by
   term coverage with a title match boosted. Worse than BM25, still transformative
   compared to substring matching.
4. Independently of either, cap lines returned per page (3 is a reasonable start) so
   breadth survives the budget.

**Contract to preserve.** `search_pages` returns matching **lines with line numbers**,
and the app parses the text block for that shape. Ranking may reorder results; it must
not change the shape. `structuredContent.hits` rides along for UI consumers.

**Tests.** A new battery pinning: a question-shaped query now matches, ranking puts
the better page first in a constructed case, per-page capping preserves breadth when
one page matches many lines, and the existing `search_pages` output shape is unchanged.
Adding it means adding it to **both** `package.json`'s `test` script and `ci.yml`, or
`pnpm test:wiring` fails.

**Risk.** This is the read path for `search_pages`, `find_inbound_links`, `validate`,
and `view_graph`. Migrations are the half a rollback cannot undo, so the schema change
must be additive and backward-compatible with the still-running old code for the
deploy window.
